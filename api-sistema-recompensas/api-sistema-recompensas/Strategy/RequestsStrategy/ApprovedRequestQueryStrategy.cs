...
public class ApprovedRequestQueryStrategy(Context context) : IRequestQueryStrategy<Request>
{
    ...
    public async Task<PaginacaoDto<Request>> ExecuteQuery(DateTime data, int pagina, int tamanhoPagina)
    {
        List<Request> requests = await _context.Request
            .Where(x => x.CreationDate.Date == data.Date && x.StatusRequest == StatusRequest.APROVADO)
